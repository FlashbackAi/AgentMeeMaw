"""Thin SQS wrapper for the tribute_render queue (long-poll receive + ack).

Trigger-only payload (queues.tribute_render):
    {"job_id", "tribute_id", "person_id", "composed_at", "enqueued_at"}
"""
...
class SQSClient:
    ...
    def receive(self, *, max_messages: int = 1,
                wait_seconds: int = 20) -> list[TributeRenderMessage]:
