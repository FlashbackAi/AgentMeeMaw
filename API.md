...
Curation preview for the pick-your-moments flow (spec 2026-07-05).
**Read-only** — nothing is minted or enqueued; confirm via
...
Ordering: picked moments first (curation rank order), then the rest of the
qualifying pool. For the 5 grid collections `picked` comes from one big-LLM
curation pass over the whole pool (assigns all five at once), cached in
Valkey per pool fingerprint — the **first** preview per pool snapshot pays
~15s; repeats (any collection) are instant. For `wisdom` every moment is
`picked: true` and no curation runs. `suggested_collection` is the grid slug
curation assigned the moment to (null if unassigned / wisdom). `used_in`
lists collections of this person's **complete** storybooks already using the
moment — render as an "also appears in X" warning chip, never blocking.
`bounds.min_select` is 5, relaxed to 3 when the whole pool is under 5.

- `400` — unknown collection. `404` — person not found.
- `409` — too few qualifying moments (same floor as create).
- `502` — the curation LLM call failed (retry later).
...
selection from the preview. Every id must come from the preview's pool
(else `400`); after dedupe the count must satisfy the preview's `bounds`
(min 5 — or 3 when the pool is under 5 — max 25, else `409`). When present
the worker renders from exactly this slice and skips its own curation;
regenerate/edit preserve the slice. **Absent = auto-curate exactly as
before.**
...
- `404` — person not found.
- `409` — too few qualifying moments (detail is user-facing: "Not enough
  stories yet — keep sharing memories of {name} …"), a `moment_ids`
  selection outside the min/max bounds, or the supplied `storybook_id`
  already exists (mint a fresh id + fresh URLs and retry).
