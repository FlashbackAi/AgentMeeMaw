...
The pool is **collection-scoped** (design 2026-07-06): for the 5 grid
collections it is the moments tagged to that collection in
`moments.storybook_collections`; for `wisdom` it is the whole qualifying pool.
There is no LLM curation call — `picked` is deterministic (the tagged pool,
chronological, with moments already used in a completed book demoted to the
back, capped at 25), so previews are instant. Ordering: picked first, then any
remainder. `collections` is the moment's full tag list; `suggested_collection`
(deprecated — kept for Node compat) is the first tag other than the previewed
collection, as a cross-book "also fits" hint. `used_in` lists collections of
this person's **complete** storybooks already using the moment — render as an
"also appears in X" chip, never blocking. `bounds.min_select` is 5 for grid
collections, relaxed to 3 for `wisdom` when its pool is under 5.

- `400` — unknown collection. `404` — person not found.
- `409` — too few qualifying moments for this collection (grid floor 5,
  wisdom floor 3; same floor as create).
...
selection from the preview. Every id must come from the preview's
collection-scoped pool (else `400`); after dedupe the count must satisfy the
preview's `bounds` (grid min 5; wisdom min 3 when its pool is under 5; max 25,
else `409`). When present the worker renders from exactly this slice;
regenerate/edit preserve the slice. **Absent = auto-select** the collection's
tagged pool deterministically (design 2026-07-06 retired the render-time
curation LLM — the route resolves the definitive slice and writes it to
`scene_moment_ids` before enqueuing, so a collection can never be rendered
from moments that don't fit it).
