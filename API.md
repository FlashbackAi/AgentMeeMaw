...
- `400` — unknown collection, or `page_put_urls` length ≠ `page_count`.
- `404` — person not found.
- `409` — too few qualifying moments (detail is user-facing: "Not enough
  stories yet — keep sharing memories of {name} …"), or the supplied
  `storybook_id` already exists (mint a fresh id + fresh URLs and retry).
