-- ============================================================================
-- 0033_tribute_render_outputs.up.sql
-- Flashback AI: Legacy Mode  -  Python-owned tribute video render
-- ----------------------------------------------------------------------------
-- The tribute output moves from Node's renderer to a Python worker
-- (flashback.workers.tribute_render): it renders the MP4 + a print PDF and
-- uploads via Node-minted presigned URLs. This migration adds:
--
--   * tributes.pdf_url      -- the print PDF (Node writes it, like video_url,
--                              from the key it minted, on tribute_render_complete)
--   * tributes.rendered_at  -- when the render worker committed
--   * tributes.render_error -- last render failure detail (worker-written)
--   * status 'failed'       -- a render that exhausted retries
--
-- and recreates tribute_status to expose pdf_url + rendered_at. The meter
-- math (percent / ready, answer-floor + depth weighting) is UNCHANGED from
-- 0030 -- only the surfaced columns grow. See spec
...
ALTER TABLE tributes ADD COLUMN IF NOT EXISTS rendered_at  TIMESTAMPTZ;
ALTER TABLE tributes ADD COLUMN IF NOT EXISTS render_error TEXT;

-- allow a terminal 'failed' status (render exhausted SQS retries)
ALTER TABLE tributes DROP CONSTRAINT IF EXISTS tributes_status_check;
ALTER TABLE tributes
    ADD CONSTRAINT tributes_status_check
    CHECK (status IN ('draft','ready','generating','complete','failed','superseded'));

-- recreate the view to surface pdf_url + rendered_at (meter math unchanged)
