...
fades on top, then a slow Ken Burns drift. Transitions:

  * "bleed" (default) -- the next page bleeds in through an organic watercolour
    mask, like paint spreading across the paper.
  * "turn"  -- a page-turn slide (the current page slides off, next underneath).
  * "dip"   -- a soft crossfade.

Encoded with the bundled imageio-ffmpeg (no system ffmpeg needed).
"""
...
# Output dims: multiples of 16 for libx264 (template is 899/900x1600).
OUT_W, OUT_H = 896, 1600
ZOOM_MAX = 1.05

# Seconds per phase.
T_INTRO = 0.6
T_ILLO = 1.0    # page 1 only: illustration fade-in
T_TEXT = 0.8    # line fade-in
T_HOLD = 2.4    # ken burns hold
T_TRANS = 1.1   # inter-page transition
T_OUT_HOLD = 1.2
T_OUT_FADE = 1.0
...
def _transition_frames(a: Image.Image, b: Image.Image, kind: str, n: int,
                       seed: int):
    """Yield n RGB frames morphing page ``a`` -> page ``b``."""
    w, h = a.size
    if kind == "turn":
        for k in range(n):
            t = k / max(1, n - 1)
...
class Page:
    ...
    def __init__(self, paper: Image.Image, illo_layer: Image.Image,
        ...
        self.base = Image.alpha_composite(
            paper.convert("RGBA"), illo_layer).convert("RGB")  # paper + illo
...
def render_video(pages: list[Page], out_path: str, fps: int = 30,
    ...
    def reveal_text_hold(pg: Page) -> None:
        """Line fades in, then a slow ken burns hold (text stays crisp)."""
    ...
    try:
        ...
        # page 1: illustration fades in over the paper, then line + hold
        p0 = pages[0]
        na = nfr(T_ILLO)
        for k in range(na):
        ...
        # following pages: transition carries the illustration in, then line + hold
        ...
        # outro: hold the closing page, then fade to paper
        last = pages[-1].final()
